  ...
  constructor() {
    try {
      ...
      const popoverComponent = `
        <div class="popover-profile" id="popoverComponent">
          <div class="popover-head container">
            <div class="popover-img">
              <img data-id="brioImgProfile" src="" class="img-profile" alt="">
            </div>
            <div class="popover-name container column">
              <strong data-id="brioNome"></strong>
              <small data-id="brioDocumento"></small>
              <ul class="list-none">
                <li><span data-id="brioNascimento"></span> <span data-id="brioIdade"></span>, <span data-id="brioCidade"></span>, <span data-id="brioEstado"></span></li>
                <li data-id="brioCargo"></li>
              </ul>
            </div>
          </div>
          <div class="popover-content">
            <ul class="list-none margin-bottom-10" data-id="brioListaCandidaturas">
              <li class="list-title">Candidaturas</li>
              <li><small><strong data-item="brioItemAno"></strong> - <span data-item="brioItemCandidatura"></span></small></li>
            </ul>
            <ul class="list-none" data-id="brioListaSociedades">
              <li class="list-title">Sociedades</li>
              <li> <small><strong data-item="brioItemCnpj"></strong> - <span data-item="brioItemRazaoSocial"></span> - <span data-item="brioItemTipoPessoa"></span> - <span data-item="brioItemOcupacao"></span></small></li>
            </ul>
          </div>
          <div class="popover-bottom">
            <div class="popover-options">
              <span class="list-title">Partidos filiados</span>
              <ul class="list-none container wrap" data-id="brioListaPartidos">
                <li class="label" data-item="partido"></li>
              </ul>
      ...
      document.body.insertAdjacentHTML('beforeend', popoverComponent)
    } catch (error) {
      this.error = error
      console.error(error)
    }
...
/**
      ...
      this.fetchByName(name).then(data => {
        data.forEach(item => {
          this.search(document.body, item.person.name).forEach(catchy => {
            ...
            catchy.node.innerHTML = catchy.node.innerHTML.split(' ').map(text => {

              // verifica se não está alterando texto de um href
              if (text.match(/href="[^"]*"|(\+)/gi)) {
                return text
              }

              return text.replace(
                  RegExp(catchy.matches, 'gi'),
                  `<a data-target="true" href="#">${catchy.matches}</a>`
              )
            }).join(' ')

            const __item = item
            const __updateComponent = this.updateComponent

            catchy.node.querySelectorAll('[data-target]').forEach(el => {
              tippy(el, {
                delay: 100,
                arrow: true,
                interactive: true,
                html: '#popoverComponent',
                click: true,
                onShow() {
                  __updateComponent(__item, this)
                }
              })
            })
          })
  ...
  /**
   * @name search busca nome nos nós
  ...
  updateComponent(data, node) {
    if (!node.alterado) {
      node.querySelector('[data-id="brioImgProfile"]').src = data.person.profilePic || 'https://www.tribunapr.com.br/wp-content/uploads/sites/1/2016/11/bolsonaro-825x665.jpg'
      node.querySelector('[data-id="brioNome"]').textContent = data.person.profilePic || 'Jair Bolsonaro'
      node.querySelector('[data-id="brioDocumento"]').textContent = data.person.profilePic || '00000000000'
      node.querySelector('[data-id="brioNascimento"]').textContent = data.person.profilePic || '12/01/2018'
      node.querySelector('[data-id="brioIdade"]').textContent = data.person.profilePic || '19'
      node.querySelector('[data-id="brioCidade"]').textContent = data.person.profilePic || 'Guarulhos'
      node.querySelector('[data-id="brioEstado"]').textContent = data.person.profilePic || 'SP'
      node.querySelector('[data-id="brioCargo"]').textContent = data.person.profilePic || 'Deputado Federal'
  
      const listaCandidaturas = node.querySelector('[data-id="brioListaCandidaturas"]')
      const listaCandidaturasItem = listaCandidaturas.querySelectorAll('li')[1]

      const listaSociedades = node.querySelector('[data-id="brioListaSociedades"]')
      const listaSociedadesItem = listaSociedades.querySelectorAll('li')[1]

      const listaPartidos = node.querySelector('[data-id="brioListaPartidos"]')
      const listaPartidosItem = listaPartidos.querySelector('li')
  
      data.eleicoes.forEach(eleicao => {
        var cloned = listaCandidaturasItem.cloneNode(true)
        cloned.querySelector('[data-item="brioItemAno"]').textContent = eleicao.ano_eleicao
        cloned.querySelector('[data-item="brioItemCandidatura"]').textContent = `${eleicao.sigla_ue}/${eleicao.descricao_ue} - ${eleicao.num_turno} - ${eleicao.descricao_cargo} - ${eleicao.sigla_partido}`
         
        listaCandidaturas.appendChild(cloned)
      })

      data.filiacoes.forEach(filiacao => {
        var cloned = listaPartidosItem.cloneNode(true)
        cloned.textContent = filiacao.sigla_do_partido
        listaPartidos.appendChild(cloned)
      })
    }
